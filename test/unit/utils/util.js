...
const createElm = function () {
  ...
  elm.id = `app${++id}`; // eslint-disable-line
...
export function createTest(Compo, propsData = {}, mounted = false) {
  if (propsData === true || propsData === false) {
    mounted = propsData;  // eslint-disable-line
    propsData = {}; // eslint-disable-line
  }
  const elm = createElm();
...
export function dateToString(d) {
  return [d.getFullYear(), d.getMonth() + 1, d.getDate()]
    .map(nr => nr > 9 ? nr : `0${nr}`).join('-');  // eslint-disable-line
}
 ...
 */
export function waitForIt(condition, callback) {
 ...
 */
export function promissedTick(component) {
